...
public interface FormaPagamentoRepository extends JpaRepository<FormaPagamento, Long> {
        ...
        Optional<FormaPagamento> findByIdAndEmpresa(Long id, Empresa empresa);

        void deleteByIdAndEmpresa(Long id, Empresa empresa);

        // Consulta personalizada com filtro LIKE e paginação
        @Query(value = "SELECT fp.* FROM fin_formas_pagamento fp WHERE " +
                        "(:termo IS NULL OR unaccent(fp.nome) LIKE CONCAT('%', :termo, '%')) " +
                        "AND fp.id_empresa = :idEmpresa ", nativeQuery = true)
        Page<FormaPagamento> findByTermo(@Param("termo") String termo, Long idEmpresa, Pageable pageable);
